...
n_steps,epochs,hidden_units,batch_size,verbose):
    ...
    if not os.path.exists(input_dir+ "/mlp_headed"):
        os.makedirs(input_dir+ "/mlp_headed")
    
    # for file_analysis in list_file:
    print(str(file_analysis))
    mlflow.set_tag("mlflow.runName", "mlp_headed -  " + str(file_analysis.replace(".csv","").replace("train_","")))
    ...
    plt.savefig(input_dir+"/mlp_headed/"+file_analysis.replace(".csv","") + ".png")
     
    mlflow.log_artifact(input_dir+"/mlp_headed/"+file_analysis.replace(".csv","")+".png")
